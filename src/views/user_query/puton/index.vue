<template>
  <div class="puton_body">
    ...
    <div class="puton_card">
      ...
      <div class="hr"></div>
      <div class="puton_item">
        <p class="item_text">nullkg</p>
        <p class="item_title">重量</p>
      </div>
      <div class="puton_item">
        <p class="item_text">1445</p>
        <p class="item_title">积分</p>
      </div>
    </div>
    <div class="puton_list">
      <div class="table_header">
        <span id="ljlx">垃圾类型</span>
        <span id="tfjf">投放积分</span>
        <span id="tfzl">投放重量</span>
        <span id="tfsj">投放时间</span>
      </div>
      <PullRefresh v-model="refreshing" @refresh="onRefresh" success-text="刷新成功">
        <List v-model="loading" @load="onLoad" :finished="finished" finished-text="没有更多了">
          <div v-for="(item, index) in dataList" :key="index" class="table_body">
            <span id="ljlx">{{ item.flmc }}</span>
            <span id="tfjf">{{ item.hdjf }}</span>
            <span id="tfzl">{{ item.tdzlf }}</span>
            <span id="tfsj">{{ item.tfsjStr }}</span>
          </div>
        </List>
      </PullRefresh>
    </div>
  ...
  methods: {
    ...
    getPutonList() {
      const query = {
        ...
        pageSize: 10,
        dateStart: this.date1 + ' 00:00:00',
        dateEnd: this.date2 + ' 23:59:59'
      }
      console.log(query)
      // 接口请求
      for (let i = 0; i < 16; i++) {
